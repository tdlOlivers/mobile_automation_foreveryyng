...
And(/^I go to the (\d). top searched item$/) do |index|
  @screens.get_current.click_on_search_field
  @screens.get_current.open_top_searched_item_by_index(index.to_i)
end

When(/^I click hearts of first (\d+) items and memorize their names$/) do |count|
  @screens.product_screen.wait_to_load
  @product_titles = @screens.get_current.add_x_items_to_the_wishlist(count.to_i)
end

And(/^I return to the home screen$/) do
  @screens.get_current.return_to_home
end

And(/^I click on Wishlist tab$/) do
  @screens.get_current.open_wishlist_tab
end

Then(/^I see all items which I previously added to the wishlist$/) do
  @screens.get_current.validate_wishlist(@product_titles)
end
